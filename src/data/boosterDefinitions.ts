...
function parseIngredients(ingredientStrings: string[]): Ingredient[] {
	...
	return ingredientStrings.map(str => {
		...
		return { type: 'UNKNOWN', quantity: 0 }; // Handle unparsable strings, or throw error
	}).filter(item => item.type !== 'UNKNOWN'); // Filter out unknowns
}


export const allBoosters: Booster[] = [
	{
		id: 'temporalTweaker', // Using your 'varname'
		name: 'Temporal Tweaker',
		description: 'Temporal Tweaker: reset the cooldown phase for a booster your choice',
		runtime: 0 * 60, // 0 minutes runtime -> 0 seconds
		cooldown: 0 * 60, // This seems to imply it's instant and has no cooldown, but it might mean 'no cooldown for itself, but affects others'. Let's keep it 0 as per your definition.
		boosterUseCountName: 'boosterUseCountTemporalTweaker',
		boosterDateSettingKey: 'boosterDateTemporalTweaker',
		boosterFactorSwitchSettingKey: '', // No factor switch for this one
  ...
  <path d="M15.03 8.97a7 7 0 0 0-6.42-.03L9.5 7.5l-3 3 3 3 .71-.71-1.07-1.07a5.5 5.5 0 0 1 5.37-.02l.71.71 3-3-3-3-.71-.71z" fill="currentColor"/>
		...
		color: '#4CAF50' // Dark Green (from previous suggestion)
	},
	{
		...
		cooldown: 3 * 24 * 60 * 60, // 3 days -> 3 * 24 hours -> 3 * 24 * 60 minutes -> 3 * 24 * 60 * 60 seconds (Description says 3 days, current code has 36*60 minutes = 36 hours. I'll use 3 days as per description, you can adjust)
  ...
  <path d="M12 4L4 12h5v8h6v-8h5L12 4zM9 12v6h6v-6H9z" fill="currentColor"/>
		...
		color: '#FFC107', // Amber (from previous suggestion)
		multiplier: 3 // 3 times points
	},
	{
		...
		runtime: 4 * 60 * 60, // 4 hours -> 4 * 60 minutes -> 4 * 60 * 60 seconds
		cooldown: 5 * 24 * 60 * 60, // 5 days -> 5 * 24 hours -> 5 * 24 * 60 minutes -> 5 * 24 * 60 * 60 seconds
  ...
  <path d="M12 2C8.13 2 5 5.13 5 9c0 2.37 1.34 4.45 3.32 5.5V17H7v2h1v4h8v-4h1v-2h-1v-2.5c1.98-1.05 3.32-3.13 3.32-5.5C19 5.13 15.87 2 12 2zm-2 5c0-1.1.9-2 2-2s2 .9 2 2-.9 2-2 2-2-.9-2-2zm0 8h4v-2h-4v2zm-2-2h-2v-2h2v2zM12 22h-2v-2h2v2zm4-8h-4v-2h4v2zm2 2h-2v-2h2v2zm0-8c0-1.1-.9-2-2-2s-2 .9-2 2 .9 2 2 2 2-.9-2-2z" fill="currentColor"/>
		...
		color: '#9C27B0', // Deep Purple (from previous suggestion)
		multiplier: 3 // 3 times EP
	},
	{
  ...
  <path d="M12 2L4 12h5v8h6v-8h5L12 2zm0 17c-2.76 0-5-2.24-5-5h2c0 1.66 1.34 3 3 3s3-1.34 3-3h2c0 2.76-2.24 5-5 5z" fill="currentColor"/>
		...
		color: '#2196F3', // Blue (from previous suggestion)
		multiplier: 2 // Assuming "much faster" implies 2x rate
	},
	{
		id: 'linkersLode',
		name: 'Linkers Lode',
		description: 'Linkers Lode: collect extra points when creating chain of thoughts for the next 3 hours. (cooldown 2 days)',
		runtime: 3 * 60 * 60, // 3 hours -> 3 * 60 minutes -> 3 * 60 * 60 seconds
		cooldown: 2 * 24 * 60 * 60, // 2 days -> 2 * 24 hours -> 2 * 24 * 60 minutes -> 2 * 24 * 60 * 60 seconds
  ...
  <path d="M17 7h-4V4H7v10h4v3h7v-7h-4zM7 7h2v2H7V7zm4 10H9v-2h2v2zm0-6H9V9h2v2zm0-4H9V5h2v2zm4 4h-2V9h2v2zm0-4h-2V5h2v2zM15 13h2v-2h-2v2z" fill="currentColor"/>
		...
		color: '#E91E63', // Pink (from previous suggestion)
		multiplier: 2 // Assuming "extra points" implies 2x as per previous description
	},
	{
		...
		runtime: 5 * 24 * 60 * 60, // 5 days -> 5 * 24 hours -> 5 * 24 * 60 minutes -> 5 * 24 * 60 * 60 seconds
		cooldown: 20 * 24 * 60 * 60, // 20 days -> 20 * 24 hours -> 20 * 24 * 60 minutes -> 20 * 24 * 60 * 60 seconds
		boosterUseCountName: 'boosterUseCountEffortlessExpansion',
		boosterDateSettingKey: 'boosterDateEffortlessExpansion',
		boosterFactorSwitchSettingKey: '', // No factor switch in code, but description implies an effect
  ...
  <path d="M12 2L4 12h5v8h6v-8h5L12 2zm0 17c-2.76 0-5-2.24-5-5h2c0 1.66 1.34 3 3 3s3-1.34 3-3h2c0 2.76-2.24 5-5 5z" fill="currentColor"/>
		...
		color: '#00BCD4', // Cyan (from previous suggestion)
		pointReductionFactor: 0.20 // 20% reduction
	},
	{
		...
		runtime: 5 * 60 * 60, // 5 hours -> 5 * 60 minutes -> 5 * 60 * 60 seconds
		cooldown: 3 * 24 * 60 * 60, // 3 days -> 3 * 24 hours -> 3 * 24 * 60 minutes -> 3 * 24 * 60 * 60 seconds
		boosterUseCountName: 'boosterUseCountrEcursiveReflection', // Note the 'rE' in your existing name
  ...
  <path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z" fill="currentColor"/>
		...
		color: '#FF5722', // Deep Orange (from previous suggestion)
		multiplier: 5 // 5 times points
	},
	{
		id: 'synapticSurge',
		name: 'Synaptic Surge',
		...
		runtime: 2 * 60 * 60, // 2 hours -> 2 * 60 minutes -> 2 * 60 * 60 seconds
		cooldown: 36 * 60 * 60, // 36 hours (your code has 36*60 minutes)
  ...
  <path d="M13 14h-2v-4h2v4zM13 18h-2v-2h2v2zM13 6h-2V4h2v2zM17 14h-2v-4h2v4zM17 18h-2v-2h2v2zM17 6h-2V4h2v2zM9 14h-2v-4h2v4zM9 18h-2v-2h2v2zM9 6h-2V4h2v2zM21 10h-2V8h2v2zM5 10H3V8h2v2zM15 22h-2v-2h2v2zM15 2h-2V0h2v2zM7 22h-2v-2h2v2zM7 2h-2V0h2v2zM19 22h-2v-2h2v2zM19 2h-2V0h2v2z" fill="currentColor"/>
		...
		color: '#FFEB3B', // Yellow (from previous suggestion)
		multiplier: 20 // 20 times points
	},
	{
		...
		runtime: 2 * 24 * 60 * 60, // 2 days -> 2 * 24 hours -> 2 * 24 * 60 minutes -> 2 * 24 * 60 * 60 seconds
		cooldown: 48 * 60 * 60, // 48 hours (your code has 48*60 minutes)
  ...
  <path d="M12 2C8.69 2 6 4.69 6 8.5V14c0 1.1-.9 2-2 2v1h16v-1c-1.1 0-2-.9-2-2V8.5C18 4.69 15.31 2 12 2zm0 13l-1.5-1.5-1.5 1.5V9h3v5.5zM9 21c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1H9v1z" fill="currentColor"/>
		...
		color: '#673AB7', // Deep Purple 2 (from previous suggestion)
		frequencyIncrease: 0.40 // Assuming 40% based on previous description
	},
	{
		id: 'titleTitan',
		name: 'Title Titan',
  ...
  <path d="M5 4v3h5.5v12h3V7H19V4H5z" fill="currentColor"/>
		...
		color: '#F44336', // Red (from previous suggestion)
		multiplier: 4 // Quadro points -> 4x
	},
	{
		id: 'precisionPrism',
  ...
  <path d="M11 20H6V4h5v16zm-1-1v-1H7v1h3zm0-3v-1H7v1h3zm0-3v-1H7v1h3zm0-3v-1H7v1h3zm0-3V9H7v1h3zm0-3V6H7v1h3zm7 12h-5V4h5v16zm-1-1v-1h-3v1h3zm0-3v-1h-3v1h3zm0-3v-1h-3v1h3zm0-3v-1h-3v1h3zm0-3V9h-3v1h3zm0-3V6h-3v1h3z" fill="currentColor"/>
		...
		color: '#795548', // Brown (from previous suggestion)
		multiplier: 4 // 4 times points
	},
	{
		id: 'hyperlinkHarmony',
  ...
  <path d="M10 13c0 1.1-.9 2-2 2H4v2h4c2.21 0 4-1.79 4-4V7h2v6zm4-6V4h-4C9.79 4 8 5.79 8 8v2H6V8c0-1.1.9-2 2-2h6z" fill="currentColor"/>
		...
		color: '#607D8B', // Blue Grey (from previous suggestion)
		multiplier: 5 // 5 times points
	},
	{
		...
		runtime: 2 * 60 * 60, // 2 hours -> 2 * 60 minutes -> 2 * 60 * 60 seconds
		cooldown: 2160 * 60 * 60, // 2160 hours (your code has 2160*60 minutes)
		...
		specialIngredientRequirement: 'all pots', // From description
		svg: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
  ...
  <path d="M12 1L9 9l-8 1 7 7-2 8 8-2 7 2-2-8 7-7-8-1L12 1z" fill="currentColor"/>
		...
		color: '#FFD54F', // Light Yellow (from previous suggestion)
		multiplier: 5 // Assuming 'astonishing rate' means 5x as per previous suggestion
	},
	{
		id: 'fortuneInfusion',
		...
		runtime: 0 * 60, // No specific runtime mentioned for *this* booster's effect directly
		cooldown: 22 * 60 * 60, // 22 hours (your code has 22*60 minutes)
		...
		ingredients: [], // No explicit ingredients for crafting mentioned
		specialIngredientRequirement: 'free all 22h', // From description
		svg: `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
  ...
  <path d="M6 3h12c1.1 0 2 .9 2 2v14c0 1.1-.9 2-2 2H6c-1.1 0-2-.9-2-2V5c0-1.1.9-2 2-2zm12 2H6v14h12V5zm-8 4v2h2V9h-2zm4 0v2h2V9h-2zm-4 4v2h2v-2h-2zm4 0v2h2v-2h-2zM12 7l-2 2 2 2 2-2-2-2z" fill="currentColor"/>
		...
		color: '#CDDC39' // Lime Green 2 (from previous suggestion)
		// No direct multiplier, effect is random ingredient granting
	}
];
